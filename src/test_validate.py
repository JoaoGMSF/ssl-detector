import plot
from field_detection import FieldDetection
import subprocess
import numpy as np
import math
import glob
import cv2
import os

if __name__ == "__main__":

    cwd = os.getcwd()

    FRAME_NR = 600
    QUADRADO = 1
    WINDOW_NAME = "BOUNDARY DETECTION"
    VERTICAL_LINES_NR = 60

    # FIELD DETECTION TESTS
    field_detector = FieldDetection(
                    vertical_lines_offset=320,
                    vertical_lines_nr=15,
                    min_line_length=1,
                    max_line_length=20,
                    min_wall_length=15,
                    arrange_random=False)
    print(field_detector.vertical_lines)


    # Specify the directory path for which you want to get subdirectories
    directory_path = 'data/pibic_evaluate'

    # Initialize an empty list to store the directory paths
    subdirectories = []

    # Use os.listdir() to get a list of all items (files and directories) in the specified directory
    for item in os.listdir(directory_path):
        item_path = os.path.join(directory_path, item)
        
        # Check if the item is a directory using os.path.isdir()
        if os.path.isdir(item_path):
            subdirectories.append(item_path)

    for directory_path in subdirectories:

        archive_extensions = ['.png'] 

        archive_paths = []

        for extension in archive_extensions:
            pattern = os.path.join(directory_path, f"*{extension}")
            archive_paths.extend(glob.glob(pattern))

        for path in archive_paths:

            IMG_PATH = cwd + f"/{path}"

            WINDOW_NAME = IMG_PATH

            img = cv2.imread(IMG_PATH)
            first_lines_img = img.copy()

            preprocessed = field_detector.preprocess(first_lines_img, field_detector.vertical_lines)
            print_img = preprocessed.copy()
            segmented_img = field_detector.segmentField(preprocessed, field_detector.vertical_lines)
            boundary_points, orientation, sobel_img = field_detector.fieldWallDetection(segmented_img, img)

            ymean_boundary_points = np.mean([point[0] for point in boundary_points])
            xmean_boundary_points = np.mean([point[1] for point in boundary_points])
            orientation_mean = np.mean(orientation)

            def draw_sobel_line(img, y_mean, x_mean, orientation_mean, color=[255, 0, 0], thickness=2):
                a = -math.tan(np.deg2rad(orientation_mean))
                x0 = x_mean 
                y0 = y_mean
                x1 = 0  
                x2 = 640
                y1 = int((x1-x0)*a+y_mean)
                y2 = int((x2-x0)*a+y_mean)
                cv2.line(img,(x1,y1),(x2,y2),color,thickness)   

            def validate(y_mean, x_mean, orientation_mean):

                a = -math.tan(np.deg2rad(orientation_mean))
                sum_difs = 0

                for point in boundary_points:
                    y_line = (point[1]-x_mean)*a + y_mean
                    y_point = point[0]
                    sum_difs += (y_line-y_point)**2
                    # breakpoint()
                    print(f"y_line = {y_line}; y_point = {y_point}")

                rmse = math.sqrt(sum_difs/len(boundary_points))

                return rmse

            draw_sobel_line(sobel_img, ymean_boundary_points, xmean_boundary_points, orientation_mean+90)

            rmse = validate(ymean_boundary_points,xmean_boundary_points,orientation_mean+90)

            field_detector.boundary = boundary_points

            print(f"rmse = {rmse}")

            for point in boundary_points:
                pixel_y, pixel_x = point
                sobel_img[pixel_y, pixel_x] = field_detector.BLUE
            
            cv2.imshow(WINDOW_NAME, sobel_img)

            subprocess.call(['xdotool', 'search', '--name', WINDOW_NAME, 'windowactivate'])

            key = cv2.waitKey(-1) & 0xFF
            if key == ord('q'):
                plot.plt.close('all')
                break
            else:
                plot.plt.close('all')
                FRAME_NR += 1

        # RELEASE WINDOW AND DESTROY
    cv2.destroyAllWindows()